export default {
  ...
  nav: {
    ...
    logout: '退出登录',
    installApp: '安装应用',
    installAppSuccess: 'LayerCove 已安装',
  },
  ...
  printers: {
    ...
    maintenance: {
      ...
      editFieldLabel: '维护模式',
      editFieldHelp: '启用后，此打印机将从 MQTT、队列调度和通知中暂停 — 适用于维修、并行 LayerCove 安装或临时停用。',
    },
    // Chamber light
  ...
  archives: {
    ...
    no3mfBanner: {
      ...
      body: '切片机没有在打印机的 SD 卡上留下 .gcode.3mf 文件，因此 LayerCove 无法获取缩略图或切片机元数据。这通常是因为切片机 (Bambu Studio / OrcaSlicer 的设备选项卡) 中"将发送的文件存储在外部存储中"已关闭。',
      docsLink: '查看安装步骤 4',
      dismissLabel: '关闭此通知',
    },
  ...
  settings: {
    ...
    ldap: {
      ...
      feature3: '登录时 LDAP 组映射到 LayerCove 组',
    ...
    preheatFilamentTargetsHint: 'LayerCove 会在已装入的 AMS 槽位中选择最高目标；仅 PLA 的打印推导为 0 并自动跳过腔体阶段。',
    ...
    authDisabledMessage: '启用身份验证以创建用户账户、管理权限并保护您的 LayerCove 实例。',
    ...
    disableAuthenticationMessage: '确定要禁用身份验证吗？这将使您的 LayerCove 实例无需登录即可访问。所有用户将保留在数据库中但身份验证将被禁用。',
    disableAuthentication: '禁用身份验证',
    // Additional settings
    configureBambuddy: '配置 LayerCove',
    systemDefault: '系统默认',
    ...
    captureFinishPhotoDescription: '打印完成时从打印机摄像头拍照。LayerCove 会在打印期间录制一段短延时摄影，以便从热床下降前的瞬间获取照片；如果您为本次打印启用了延时摄影，文件将保留，否则会在拍照完成后自动删除。',
    ...
    bambuddySoftware: 'LayerCove 软件',
    autoCheckDescription: '启动时自动检查新版本',
    checkNow: '立即检查',
    updateAvailableVersion: '可用更新：v{{version}}',
    releaseNotes: '发布说明',
    updateViaDocker: '通过 Docker Compose 更新：',
    updateViaHomeAssistant: '更新由 Home Assistant Supervisor 管理。请在 Home Assistant 中打开 设置 → 加载项 → LayerCove 以安装新版本。',
    ...
    goToBackup: '前往备份',
    externalUrl: '外部URL',
    externalUrlDescription: 'LayerCove可访问的外部URL。用于通知图像和外部集成。',
    ...
    mqttDescription: '将LayerCove事件发布到外部MQTT代理，用于与Node-RED、Home Assistant和其他自动化系统集成。',
    ...
    encryption: {
      ...
      notConfiguredDesc: 'TOTP 密钥和 OIDC client_secrets 以明文存储。请设置 MFA_ENCRYPTION_KEY 或使用可写数据目录重启 LayerCove 以自动生成。',
      ...
      migrationErrorWarning: '{{count}} 行旧数据在启动时未能重新加密。请检查服务器日志并重启 LayerCove 以重试。',
    },
  ...
  login: {
    ...
    twoFA: {
      ...
      methodEmail: '邮箱验证码',
      methodBackup: '备用恢复码',
      instructionsTotp: '请打开您的身份验证器应用，输入 LayerCove 的 6 位验证码。',
  ...
  setup: {
    title: 'LayerCove 设置',
    subtitle: '为您的 LayerCove 实例配置身份验证',
    enableAuth: '启用身份验证',
    adminAccount: '管理员账户',
  ...
  users: {
    title: '用户管理',
    subtitle: '管理用户及其对 LayerCove 实例的访问',
    backToSettings: '返回设置',
    createUser: '创建用户',
    noPermission: '您没有访问此页面的权限。',
    admin: '管理员',
  ...
  profiles: {
    ...
    orcaCloud: {
      ...
      connect: {
        ...
        description: '登录您的 Orca Cloud 账户,将切片机配置同步到 LayerCove。',
      },
      providers: {
        google: '使用 Google 登录',
        apple: '使用 Apple 登录',
  ...
  sponsors: {
    ...
    tagline: 'LayerCove 完全免费，并将持续免费——这要感谢主动支持的用户。没有风投，也没有云端绑定。',
    viewSupporters: '查看支持者',
    toastPrints: '你已经用 LayerCove 完成了 {{count}} 次打印。LayerCove 之所以免费，离不开支持者。',
    toastCost: '你已经用 LayerCove 追踪了 {{total}} 的耗材。看看是谁让项目保持独立。',
    toastArchives: '用 LayerCove 归档了 {{count}} 次打印。看看是谁让它保持独立。',
    toastAnniversary: '与 LayerCove 相伴一年了！看看是谁让项目保持独立。',
    toastVersionUpdate: '已更新至 v{{version}}。LayerCove 之所以免费，离不开支持者。',
  },
  ...
  inventory: {
    ...
    spoolmanMixedContentBody: 'LayerCove 通过您的反向代理以 HTTPS 提供服务，但您的 Spoolman 地址仍为 HTTP。出于安全考虑，浏览器会阻止混合内容，因此嵌入式 Spoolman 界面无法加载。Spoolman 也必须通过 HTTPS 访问。',
    spoolmanMixedContentFixReverseProxy: '请将 Spoolman 置于与 LayerCove 相同的反向代理（Traefik / Nginx / Caddy）之后并启用 HTTPS，然后在设置中将 Spoolman URL 更新为新的 HTTPS 地址。',
  ...
  backup: {
    ...
    repoIsPublicWarning: '仓库为公开（PUBLIC）。LayerCove 备份包含 MQTT 凭据、Home Assistant 令牌、Prometheus 令牌、您的 Bambu Cloud 邮箱以及通过 K-profile 暴露的打印机访问代码。在您于提供商设置中将仓库改为私有之前，保存将被阻止。',
    repoVisibilityUnknown: '无法确定仓库的可见性。LayerCove 拒绝向任何未确认为私有的目标进行备份；保存将被阻止。',
    ...
    appSettings: '应用设置',
    appSettingsDescription: 'LayerCove 配置（完整数据库）',
    spoolInventory: '耗材库存',
    ...
    // Local Backup
    localBackup: '本地备份',
    localBackupDescription: '创建 LayerCove 数据的完整备份，包括数据库、档案、上传和所有文件。',
    ...
    backupRestoredRestart: '备份已恢复。请重启 LayerCove。',
  ...
  kProfiles: {
    ...
    modal: {
      ...
      notesHelp: '备注保存在 LayerCove 中，不在打印机上',
      syncing: '与打印机同步中...',
  ...
  virtualPrinter: {
    ...
    enable: {
      title: '启用虚拟打印机',
      visibleInSlicer: '在切片软件发现中显示为"LayerCove"',
      proxyingTo: '代理到 {{name}}',
      notActive: '未激活',
    },
    model: {
    ...
    remoteInterface: {
      ...
      placeholder: '自动检测（默认）...',
      hint: '覆盖通过 SSDP 广播并在 TLS 证书中使用的 IP 地址。在 LayerCove 有多个网络接口时很有用。',
    },
    mode: {
      title: '模式',
      archive: '归档',
    ...
    howItWorks: {
      ...
      step2: '在归档、审核和队列模式下，使用切片软件中的"发送"按钮将 3MF 文件上传到 LayerCove。切片软件会显示"打印成功"— 文件已存储，未打印。',
  ...
  diagnostic: {
    ...
    check: {
      ...
      network_mode: {
        ...
        warn: 'LayerCove 正在以 Docker bridge 网络运行。打印机发现和虚拟打印机需要 host 网络模式 — 请使用 "network_mode: host" 重新创建容器。',
        skip: '未在 Docker 中运行 — 不适用。',
      },
      subnet: {
        title: '网络子网',
        pass: '打印机和 LayerCove 位于同一子网。',
        warn: '打印机（{{printer_ip}}）和 LayerCove（{{host_ip}}）位于不同的子网。除非配置了子网之间的路由，否则它们可能无法相互访问。',
        skip: '无法确定子网 — 已跳过。',
      },
      mqtt_auth: {
        title: '打印机凭据',
  ...
  systemHealth: {
    ...
    signature: {
      'ftp-auth-rejected': {
        ...
        fix: '从打印机屏幕（局域网设置）重新复制访问码，并在 LayerCove 的打印机设置中更新它。',
      },
      'ftp-connection-timeout': {
        name: '文件传输连接超时',
        cause: 'LayerCove 无法连接打印机的文件传输端口（FTPS 990）。该端口被阻止，或打印机已关机或位于其他子网。',
        fix: '请确保 LayerCove 与打印机之间的 990 端口未被阻止，并且两者位于同一网络。',
      },
      'ftp-ssl-error': {
        name: '安全文件传输握手失败',
      ...
      'database-locked': {
        ...
        fix: '请将 LayerCove 切换到外部 PostgreSQL 数据库。参见文档中的 PostgreSQL 指南。',
      },
    },
  },
  ...
  vpDiagnostic: {
    ...
    check: {
      ...
      running: {
        title: '服务正在运行',
        fail: '虚拟打印机已启用，但其服务未运行。请检查 LayerCove 日志 — 通常是绑定 IP 冲突或权限错误使其停止。',
      },
      bind_interface: {
        title: '绑定网络接口',
      ...
      certificate: {
        ...
        fail: '此虚拟打印机的 TLS 证书缺失。请检查 LayerCove 数据目录是否可写。',
      },
    },
  },
  ...
  failureDetection: {
    ...
    externalUrlHint: 'ML API 通过 URL 获取摄像头快照。请在通用设置中设置外部 URL，以便 ML API 容器可以访问 LayerCove。',
  ...
  makerworld: {
    ...
    disclaimer: 'MakerWorld 集成使用由社区记录的 API 接口。LayerCove 与 MakerWorld 或 Bambu Lab 没有从属或认可关系。',
  ...
  libraryPurge: {
    ...
    effect4: '外部（链接）文件夹中的文件将被跳过 — LayerCove 从不删除不属于自己的字节。',    previewLoading: '正在检查匹配的文件数量…',
