export default {
  ...
  nav: {
    ...
    installAppSuccess: 'LayerCoveをインストールしました',
  },
  ...
  printers: {
    ...
    maintenance: {
      ...
      editFieldHelp: '有効にすると、このプリンターはMQTT、キューディスパッチ、通知から一時停止されます — 修理、並列のLayerCoveインストール、または一時的な停止に役立ちます。',
    },
    // Chamber light
  ...
  archives: {
    ...
    no3mfBanner: {
      ...
      body: 'スライサーがプリンターのSDカードに.gcode.3mfファイルを残さなかったため、LayerCoveはサムネイルもスライサーメタデータも取得できませんでした。これは通常、スライサー (Bambu Studio / OrcaSlicer のデバイスタブ) で「送信ファイルを外部ストレージに保存」がオフになっていることが原因です。',
  ...
  settings: {
    ...
    ldap: {
      ...
      feature3: 'ログイン時にLDAPグループがLayerCoveグループにマッピングされます',
      ...
      autoProvisionHint: '初回LDAPログイン時にLayerCoveアカウントを自動作成',
      ...
      groupMappingHint: 'LDAPグループDNをLayerCoveグループにマッピング。利用可能なグループ: ',
    ...
    preheatFilamentTargetsHint: 'LayerCoveはロード済みのAMSスロット中で最も高い目標値を選びます。PLAのみの印刷は0となり、チャンバー段階は自動でスキップされます。',
    ...
    authDisabledDescription: '認証を有効にして、ユーザーアカウントの作成、権限の管理、LayerCoveインスタンスのセキュリティを確保しましょう。',
    authDisabledMessage: '認証を有効にして、ユーザーアカウントの作成、権限の管理、LayerCoveインスタンスのセキュリティを確保しましょう。',
    ...
    disableAuthenticationMessage: '認証を無効にしますか？LayerCoveインスタンスにログインなしでアクセスできるようになります。ユーザーはデータベースに残りますが、認証は無効になります。',
    disableAuthentication: '認証を無効化',
    // Additional settings
    configureBambuddy: 'LayerCoveを設定',
    ...
    captureFinishPhotoDescription: '印刷完了時にプリンターカメラから写真を撮影します。LayerCove は印刷中に短いタイムラプスを記録し、ベッドが下がる前の瞬間から写真を取得できるようにします。この印刷でタイムラプスを有効にしていた場合はタイムラプスファイルが保存され、それ以外の場合は写真の取得後に自動的に削除されます。',
    ...
    bambuddySoftware: 'LayerCoveソフトウェア',
    autoCheckDescription: '起動時に自動的に新しいバージョンを確認',
    checkNow: '今すぐ確認',
    updateAvailableVersion: 'アップデート利用可能: v{{version}}',
    releaseNotes: 'リリースノート',
    updateViaDocker: 'Docker Composeでアップデート:',
    updateViaHomeAssistant: 'アップデートはHome Assistant Supervisorによって管理されます。Home Assistantの設定→アドオン→LayerCoveを開いて新しいバージョンをインストールしてください。',
    ...
    // Network tab
    externalUrl: '外部URL',
    externalUrlDescription: 'LayerCoveがアクセス可能な外部URL。通知画像や外部連携に使用されます。',
    bambuddyUrl: 'LayerCoveのURL',
    ...
    mqttDescription: 'Node-RED、Home Assistant、その他の自動化システムとの統合のため、外部MQTTブローカーにLayerCoveイベントをパブリッシュ。',
    ...
    encryption: {
      ...
      notConfiguredDesc: 'TOTPシークレットとOIDCのclient_secretsは平文で保存されています。MFA_ENCRYPTION_KEYを設定するか、書き込み可能なデータディレクトリでLayerCoveを再起動して自動生成してください。',
      ...
      migrationErrorWarning: '{{count}} 件のレガシー行を起動時に再暗号化できませんでした。サーバーログを確認し、LayerCove を再起動して再試行してください。',
    },
  ...
  login: {
    ...
    twoFA: {
      ...
      instructionsTotp: '認証アプリを開いて、LayerCove用の6桁のコードを入力してください。',
  ...
  // Setup page
  setup: {
    title: 'LayerCove セットアップ',
    subtitle: 'LayerCoveインスタンスの認証を設定',
    enableAuth: '認証を有効化',
  ...
  users: {
    title: 'ユーザー管理',
    subtitle: 'ユーザーとLayerCoveインスタンスへのアクセスを管理',
    backToSettings: '設定に戻る',
    createUser: 'ユーザーを作成',
    noPermission: 'このページにアクセスする権限がありません。',
    admin: '管理者',
  ...
  profiles: {
    ...
    orcaCloud: {
      ...
      connect: {
        title: 'Orca Cloudに接続',
        description: 'Orca Cloudアカウントにサインインして、スライサープロファイルをLayerCoveに同期します。',
      },
      providers: {
        google: 'Googleでサインイン',
        apple: 'Appleでサインイン',
  ...
  sponsors: {
    ...
    tagline: 'LayerCoveは無料で、これからも無料です。支援してくださる方々のおかげで、VCもクラウドロックインもありません。',
    viewSupporters: 'サポーター一覧',
    toastPrints: 'LayerCoveで{{count}}件の印刷を完了しました。LayerCoveは支援者のおかげで無料で提供されています。',
    toastCost: 'LayerCoveで{{total}}分のフィラメントを記録しました。プロジェクトを支えてくれている方々をご覧ください。',
    toastArchives: '{{count}}件の印刷をLayerCoveでアーカイブしました。独立を支えてくれている方々をご覧ください。',
    toastAnniversary: 'LayerCoveとの1周年です！プロジェクトを支えてくれている方々をご覧ください。',
    toastVersionUpdate: 'v{{version}}にアップデートされました。LayerCoveは支援者のおかげで無料で提供されています。',
  },
  ...
  inventory: {
    ...
    spoolmanMixedContentBody: 'LayerCove はリバースプロキシ経由で HTTPS 配信されていますが、Spoolman の URL は HTTP のままです。ブラウザはセキュリティ上の理由で混在コンテンツをブロックするため、埋め込みの Spoolman UI を表示できません。Spoolman も HTTPS でアクセスできる必要があります。',
    spoolmanMixedContentFixReverseProxy: 'Spoolman を LayerCove と同じリバースプロキシ（Traefik / Nginx / Caddy）の後ろに HTTPS で配置し、設定で Spoolman URL を新しい HTTPS アドレスに更新してください。',
  ...
  backup: {
    ...
    repoIsPublicWarning: 'リポジトリが公開（PUBLIC）です。LayerCoveのバックアップには MQTT 認証情報、Home Assistant トークン、Prometheus トークン、Bambu Cloud のメールアドレス、K-プロファイル経由のプリンタアクセスコードが含まれます。プロバイダー側でリポジトリをプライベートに変更するまで保存はブロックされます。',
    repoVisibilityUnknown: 'リポジトリの公開設定を確認できませんでした。LayerCove はプライベートと確認できないリポジトリへのバックアップを拒否します。保存はブロックされます。',
    ...
    appSettings: 'アプリ設定',
    appSettingsDescription: 'LayerCove設定（データベース全体）',
    ...
    localBackupDescription: 'データベース、アーカイブ、アップロード、すべてのファイルを含むLayerCoveデータの完全なバックアップを作成します。',
    ...
    backupRestoredRestart: 'バックアップが復元されました。LayerCoveを再起動してください。',
  ...
  kProfiles: {
    ...
    modal: {
      ...
      notesHelp: 'メモはLayerCoveに保存され、プリンターには保存されません',
      syncing: 'プリンターと同期中...',
  ...
  virtualPrinter: {
    ...
    enable: {
      title: '仮想プリンターを有効化',
      visibleInSlicer: 'スライサーの検出リストに「LayerCove」として表示',
    ...
    remoteInterface: {
      ...
      hint: 'SSDPで広告され、TLS証明書に使用されるIPアドレスを上書きします。LayerCoveに複数のネットワークインターフェースがある場合に便利です。',
    },
    mode: {
      title: 'モード',
      archive: 'アーカイブ',
    ...
    howItWorks: {
      ...
      step2: 'アーカイブ、レビュー、キューモードでは、スライサーの「送信」ボタンを使用して3MFファイルをLayerCoveにアップロードします。スライサーは「印刷成功」と表示しますが、ファイルは保存され、印刷はされません。',
  ...
  diagnostic: {
    ...
    check: {
      ...
      network_mode: {
        ...
        warn: 'LayerCoveはDockerブリッジネットワークで実行されています。プリンター検出と仮想プリンターにはホストネットワークモードが必要です — "network_mode: host" でコンテナを再作成してください。',
        skip: 'Dockerで実行されていません — 該当しません。',
      },
      subnet: {
        title: 'ネットワークサブネット',
        pass: 'プリンターとLayerCoveは同じサブネットにあります。',
        warn: 'プリンター ({{printer_ip}}) とLayerCove ({{host_ip}}) は異なるサブネットにあります。サブネット間のルーティングが設定されていない限り、互いに到達できない可能性があります。',
  ...
  systemHealth: {
    ...
    signature: {
      'ftp-auth-rejected': {
        ...
        fix: 'プリンター画面（LAN設定）からアクセスコードをコピーし直し、LayerCove のプリンター設定で更新してください。',
      },
      'ftp-connection-timeout': {
        name: 'ファイル転送の接続がタイムアウトしました',
        cause: 'LayerCove がプリンターのファイル転送ポート（FTPS 990）に到達できませんでした。ポートがブロックされているか、プリンターの電源が切れているか、別のサブネットにあります。',
        fix: 'LayerCove とプリンターの間でポート 990 がブロックされていないこと、両者が同じネットワークにあることを確認してください。',
      },
      'ftp-ssl-error': {
      ...
      'database-locked': {
        ...
        fix: 'LayerCove を外部の PostgreSQL データベースに切り替えてください。ドキュメントの PostgreSQL ガイドを参照してください。',
      },
    },
  },
  ...
  vpDiagnostic: {
    ...
    check: {
      ...
      running: {
        title: 'サービスが稼働中',
        fail: '仮想プリンターは有効ですが、サービスが稼働していません。LayerCove のログを確認してください — 通常はバインド IP の競合または権限エラーが原因で停止します。',
      },
      bind_interface: {
      ...
      certificate: {
        ...
        fail: 'この仮想プリンターの TLS 証明書がありません。LayerCove のデータディレクトリが書き込み可能か確認してください。',
      },
    },
  },
  ...
  failureDetection: {
    ...
    externalUrlHint: 'ML APIはURLでカメラスナップショットを取得します。MLAPIコンテナがLayerCoveに到達できるよう、一般設定で外部URLを設定してください。',
  ...
  makerworld: {
    ...
    disclaimer: 'MakerWorld 連携はコミュニティで文書化された API エンドポイントを使用しています。LayerCove は MakerWorld または Bambu Lab との提携・承認関係はありません。',
  ...
  libraryPurge: {
    ...
    effect4: '外部（リンク済み）フォルダー内のファイルはスキップされます — LayerCoveは所有していないバイトを削除しません。',    previewLoading: '対象ファイル数を確認中…',
