...
describe('StreamOverlayPage', () => {
  ...
  describe('rendering', () => {
    ...
    it('shows LayerCove logo', async () => {
      renderOverlayPage(1);

      await waitFor(() => {
        expect(screen.getByAltText('LayerCove')).toBeInTheDocument();
      });
    });
    ...
    it('logo links to GitHub', async () => {
      ...
      await waitFor(() => {
        const logo = screen.getByAltText('LayerCove');
        const link = logo.closest('a');
        expect(link).toHaveAttribute('href', 'https://github.com/Timpan4/layercove');
      });
    });
  });
