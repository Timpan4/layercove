...
class TestSliceLibraryFile:
    ...
    async def test_klipper_destination_persists_raw_gcode_for_supported_inputs(
        ...
        source_id = slice_test_setup["src_file_id"]
        if source_ext != ".stl":
            source_path = slice_test_setup["tmp_path"] / "library" / "files" / f"Voron{source_ext}"
            source_path.write_bytes(_make_3mf_with_settings() if source_ext == ".3mf" else b"ISO-10303-21;")
            source = LibraryFile(
                filename=source_path.name,
                file_path=str(source_path.relative_to(slice_test_setup["tmp_path"])),
                file_type=source_ext[1:],
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            ...
            return httpx.Response(
                status_code=200,
                content=b"; generated by Custom Voron\nG28\n",
                headers={"x-print-time-seconds": "12", "x-filament-used-g": "1.5", "x-filament-used-mm": "25"},
            )

        _install_mock_sidecar(handler)
        response = await async_client.post(
            f"/api/v1/library/files/{source_id}/slice",
            json={
                "printer_preset_id": slice_test_setup["printer_id"],
                "process_preset_id": slice_test_setup["process_id"],
                "filament_preset_id": slice_test_setup["filament_id"],
        ...
        assert final["status"] == "completed", final
        assert b'name="exportType"' not in captured["body"]

        assert final["result"]["name"].endswith(".gcode")
    ...
    async def test_klipper_destination_sanitizes_stored_source_name(
        ...
        source_path = slice_test_setup["tmp_path"] / "library" / "files" / "Cube.stl"
        source = LibraryFile(
        ...
        final = await _wait_for_job(async_client, response.json()["job_id"])
        assert final["status"] == "completed", final
        assert final["result"]["name"] == "outside.gcode"

    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_klipper_persistence_stores_gcode_metadata_and_hash(self, db_session, slice_test_setup):
        ...
        response = await slice_and_persist(
            ...
            model_filename="Voron.stl",
            folder_id=None,
            extra_metadata={"sliced_from_library_file_id": slice_test_setup["src_file_id"]},
        ...
        assert persisted.filename == "Voron.gcode"
        assert persisted.file_type == "gcode"
        assert persisted.file_hash
        assert persisted.file_metadata == {
            "destination_artifact_kind": "klipper_gcode",
            "print_time_seconds": 12,
            "filament_used_g": 1.5,
            "filament_used_mm": 25.0,
            "sliced_from_library_file_id": slice_test_setup["src_file_id"],
        }
    ...
    async def test_klipper_archive_persistence_skips_3mf_artifacts(self, db_session, slice_test_setup, monkeypatch):
        ...
        source_archive = SimpleNamespace(
            ...
            printer_id=None,
            project_id=None,
            extra_data={"source": "archive"},
            print_name="Voron part",
            filament_type="PLA",
            filament_color="#ffffff",
            layer_height=0.2,
            nozzle_diameter=0.4,
            sliced_for_model="Voron 2.4",
            bed_type=None,
            makerworld_url=None,
            designer=None,
        )
        ...
        assert persisted is not None
        assert persisted.filename == "Voron.gcode"
        assert persisted.thumbnail_path is None
        assert persisted.content_hash
        assert persisted.extra_data == {
            "source": "archive",
            "destination_artifact_kind": "klipper_gcode",
            "sliced_from_archive_id": 99,
            "print_time_seconds": 12,
            "filament_used_g": 1.5,
            "filament_used_mm": 25.0,
        }
