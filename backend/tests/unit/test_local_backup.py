...
class TestCalculateNextRun:
    ...
    def test_zoneinfo_completely_unavailable_falls_back_to_stdlib_utc(self, monkeypatch):
        """The fallback remains valid when the host has no IANA timezone database
        and even ``ZoneInfo("UTC")`` raises ``ZoneInfoNotFoundError``.
        """
...
class TestPruneBackups:
    ...
    def test_prune_keeps_retention_count(self, tmp_path):
        ...
        for i in range(5):
            f = tmp_path / f"layercove-backup-20260412-{i:06d}.zip"
            f.write_text(f"backup{i}")
        service._prune_backups(tmp_path, retention=3)
        remaining = list(tmp_path.glob("layercove-backup-*.zip"))
...
class TestResolveBackupFile:
    ...
    def test_valid_filename(self, tmp_path):
        service = LocalBackupService()
        f = tmp_path / "layercove-backup-20260412-120000.zip"
        f.write_text("data")
        result = service.resolve_backup_file(str(tmp_path), "layercove-backup-20260412-120000.zip")
        assert result == f
...
class TestDeleteBackup:
    ...
    def test_delete_valid_backup(self, tmp_path):
        ...
        f = tmp_path / "layercove-backup-20260412-120000.zip"
        f.write_text("data")
        result = service.delete_backup(str(tmp_path), "layercove-backup-20260412-120000.zip")
...
class TestListBackups:
    ...
    def test_list_only_matching_files(self, tmp_path):
        service = LocalBackupService()
        (tmp_path / "layercove-backup-20260412-120000.zip").write_text("a")
        (tmp_path / "layercove-backup-20260412-130000.zip").write_text("bb")
        (tmp_path / "other-file.txt").write_text("ccc")
        result = service.list_backups(str(tmp_path))
        assert len(result) == 2
        assert all(r["filename"].startswith("layercove-backup-") for r in result)

    def test_list_sorted_newest_first(self, tmp_path):
        import time

        service = LocalBackupService()
        f1 = tmp_path / "layercove-backup-20260412-120000.zip"
        f1.write_text("a")
        time.sleep(0.05)
        f2 = tmp_path / "layercove-backup-20260412-130000.zip"
        f2.write_text("b")
        result = service.list_backups(str(tmp_path))
        assert result[0]["filename"] == "layercove-backup-20260412-130000.zip"

    def test_list_includes_size(self, tmp_path):
        service = LocalBackupService()
        (tmp_path / "layercove-backup-20260412-120000.zip").write_bytes(b"x" * 1024)
        result = service.list_backups(str(tmp_path))
