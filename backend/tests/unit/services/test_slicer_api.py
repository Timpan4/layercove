...
class TestSliceWithProfiles:
    ...
    async def test_raw_gcode_omits_export_type_and_keeps_every_profile_part(self):
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            captured["body"] = request.content
            return httpx.Response(status_code=200, content=b"; generated by OrcaSlicer\nG28\n")

        service = SlicerApiService("http://sidecar:3000", client=_mock_client(handler))
        await service.slice_with_profiles(
            model_bytes=b"x",
            model_filename="Voron.3mf",
            printer_profile_json='{"name":"Custom Voron"}',
            process_profile_json='{"name":"0.2 Voron"}',
            filament_profile_jsons=['{"name":"PLA"}', '{"name":"PVA"}'],
            plate=2,
            export_3mf=False,
        )

        body = captured["body"]
        assert b'name="exportType"' not in body
        assert b'{"name":"Custom Voron"}' in body
        assert b'{"name":"0.2 Voron"}' in body
        assert body.count(b'name="filamentProfile"') == 2
        assert body.index(b'{"name":"PLA"}') < body.index(b'{"name":"PVA"}')
