...
def extract_support_filament_slots_from_3mf(
    zf: zipfile.ZipFile,
    read_member: Callable[[str], bytes | None] | None = None,
) -> set[int]:
    ...
    try:
        content = (
            read_member("Metadata/project_settings.config")
            if read_member
            else zf.read("Metadata/project_settings.config")
        )
        if content is None:
            return set()
...
class ThreeMFDocument:
    """Read-only access to the small 3MF metadata surface slicer inputs need."""

    _MAX_METADATA_BYTES = 8 * 1024 * 1024
    _MAX_GEOMETRY_BYTES = 64 * 1024 * 1024

    def __init__(self, zf: zipfile.ZipFile):
        self._zf = zf

    def _read_member(self, path: str, max_bytes: int) -> bytes | None:
        """Read one member without expanding an unbounded payload."""
        try:
            info = self._zf.getinfo(path)
            if info.file_size > max_bytes:
                return None
            with self._zf.open(info) as member:
                content = member.read(max_bytes + 1)
        except (zipfile.BadZipFile, KeyError, OSError, RuntimeError):
            return None
        return content if len(content) <= max_bytes else None

    def _read_metadata(self, path: str) -> bytes | None:
        return self._read_member(path, self._MAX_METADATA_BYTES)

    def _read_geometry(self, path: str) -> bytes | None:
        return self._read_member(path, self._MAX_GEOMETRY_BYTES)

    def plate_count(self) -> int:
        """Return the number of plates, or ``0`` when plate metadata is unavailable."""
        content = self._read_metadata("Metadata/model_settings.config")
        if content is None:
            return 0
        xml = content.decode("utf-8", errors="replace")
        return len(re.findall(r'<metadata key="plater_id" value="(\d+)"', xml))

    def source_printer_model(self) -> str | None:
        """Return the canonical source printer model, or ``None`` when unavailable."""
        from backend.app.utils.printer_models import normalize_printer_model

        content = self._read_metadata("Metadata/project_settings.config")
        if content is None:
            return None
        try:
            cfg = json.loads(content.decode("utf-8"))
        ...
        if not raw:
            return None
        return normalize_printer_model(str(raw)) or None

    def consumed_filaments(self, plate_id: int | None = None) -> list[dict]:
        """Return positive-usage slice filaments, optionally for one plate."""
        content = self._read_metadata("Metadata/slice_info.config")
        if content is None:
            return []
        try:
            root = ET.fromstring(content)
            ...
            for plate in plates:
                ...
                for metadata in plate.findall("metadata"):
                    if metadata.get("key") == "index":
                        try:
                            plate_index = int(metadata.get("value", "0"))
        ...
        for filament in filaments:
            try:
                used_grams = float(filament.get("used_g", "0"))
            except (TypeError, ValueError):
                continue
            if math.isfinite(used_grams) and used_grams > 0:
                result.append(
                    {
                        "slot_id": filament.get("id"),
    ...
    def support_filament_slots(self) -> set[int]:
        """Return configured dedicated support-filament slots."""
        return extract_support_filament_slots_from_3mf(self._zf, self._read_metadata)

    def plate_extruder_set(self, plate_id: int) -> set[int]:
        """Return the 1-indexed extruder slots used by ``plate_id``."""
        return _extract_plate_extruder_set_from_3mf(
            self._zf,
            plate_id,
            self._read_metadata,
            self._read_geometry,
...
def _extract_plate_extruder_set_from_3mf(
    zf: zipfile.ZipFile,
    plate_id: int,
    read_member: Callable[[str], bytes | None] | None = None,
    read_geometry: Callable[[str], bytes | None] | None = None,
) -> set[int]:
    ...
    def _read(path: str) -> bytes:
        content = read_member(path) if read_member else zf.read(path)
        if content is None:
            raise OSError(f"3MF metadata member exceeds {ThreeMFDocument._MAX_METADATA_BYTES} bytes")
        return content

    if "Metadata/model_settings.config" not in zf.namelist():
        return set()
    try:
        root = ET.fromstring(_read("Metadata/model_settings.config").decode())
    ...
    if "3D/3dmodel.model" in zf.namelist():
        try:
            raw = read_geometry("3D/3dmodel.model") if read_geometry else zf.read("3D/3dmodel.model")
            if raw is None:
                raise OSError(f"3MF geometry member exceeds {ThreeMFDocument._MAX_GEOMETRY_BYTES} bytes")
            raw = raw.decode()
    ...
    def _scan_paint(path: str) -> set[int]:
        ...
        if path not in zf.namelist():
            ...
            return out
        try:
            data = read_geometry(path) if read_geometry else _read(path)
            if data is None:
                raise OSError(f"3MF geometry member exceeds {ThreeMFDocument._MAX_GEOMETRY_BYTES} bytes")
        except OSError:
            paint_cache[path] = out
...
def extract_plate_extruder_set_from_3mf(zf: zipfile.ZipFile, plate_id: int) -> set[int]:
    """Return the 1-indexed extruder slots used by ``plate_id``."""
    return ThreeMFDocument(zf).plate_extruder_set(plate_id)
