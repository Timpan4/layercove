#!/usr/bin/env bash
#
# LayerCove Native Installation Script
# Supports: Debian/Ubuntu, RHEL/Fedora/CentOS, Arch Linux, macOS
#
# Usage:
#   Interactive:  curl -fsSL https://raw.githubusercontent.com/Timpan4/layercove/main/install/install.sh -o install.sh && chmod +x install.sh && ./install.sh
#   Unattended:   ./install.sh --path /opt/layercove --port 8000 --yes
#
# Options:
#   --path PATH        Installation directory (default: /opt/layercove)
...
# Default values
DEFAULT_INSTALL_PATH="/opt/layercove"
DEFAULT_PORT="8000"
...
show_help() {
    echo "LayerCove Native Installation Script"
    echo ""
    echo "Usage: $0 [OPTIONS]"
    echo ""
    echo "Options:"
    echo "  --path PATH        Installation directory (default: /opt/layercove)"
    ...
    echo "    ./install.sh --path /srv/layercove --port 3000 --tz America/New_York --yes"
    echo ""
...
download_bambuddy() {
    log_info "Downloading LayerCove..."

    # Validate branch exists on remote before proceeding
    if ! git ls-remote --exit-code --heads https://github.com/Timpan4/layercove.git "$BRANCH" &>/dev/null; then
        log_error "Branch '$BRANCH' not found in the LayerCove repository."
        log_info "Available branches:"
        git ls-remote --heads https://github.com/Timpan4/layercove.git | sed 's|.*refs/heads/|  - |'
        exit 1
    fi
    ...
    if [[ "$OS_TYPE" == "macos" ]]; then
        ...
        if [[ -d "$INSTALL_PATH/.git" ]]; then
            ...
            git clone --branch "$BRANCH" https://github.com/Timpan4/layercove.git "$INSTALL_PATH"
        fi
        ...
        sudo git clone --branch "$BRANCH" https://github.com/Timpan4/layercove.git "$INSTALL_PATH"
        sudo chown -R "$SERVICE_USER:$SERVICE_USER" "$INSTALL_PATH" 2>/dev/null || true
    fi

    log_success "LayerCove downloaded to $INSTALL_PATH (branch: $BRANCH)"
}
...
[Unit]
Description=LayerCove - Bambu Lab Print Management
Documentation=https://github.com/Timpan4/layercove
...
EOF
    ...
    if prompt_yes_no "Enable LayerCove to start on boot?" "y"; then
        sudo systemctl enable bambuddy
        log_success "Service enabled"
    fi

    if prompt_yes_no "Start LayerCove now?" "y"; then
        sudo systemctl start bambuddy
        sleep 2
        if sudo systemctl is-active --quiet bambuddy; then
            log_success "LayerCove is running"
        else
...
EOF
    ...
    if prompt_yes_no "Load LayerCove service now?" "y"; then
...
main() {
    ...
    if [[ "$OS_TYPE" == "macos" ]]; then
        ...
        if [[ "$DEFAULT_INSTALL_PATH" == "/opt/layercove" ]]; then
            DEFAULT_INSTALL_PATH="$HOME/layercove"
        fi
    fi
    ...
    if [[ "$BIND_ADDRESS" == "0.0.0.0" ]]; then
        ...
        echo -e "  ${BOLD}Access LayerCove:${NC}  ${CYAN}http://localhost:$PORT${NC}"
    ...
    echo ""
    echo -e "  ${BOLD}Update LayerCove:${NC}"
