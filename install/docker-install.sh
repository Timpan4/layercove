#!/usr/bin/env bash
#
# LayerCove Docker Installation Script
# Supports: Linux (all distros), macOS
#
# Usage:
#   Interactive:  curl -fsSL https://raw.githubusercontent.com/Timpan4/layercove/main/install/docker-install.sh -o docker-install.sh && chmod +x docker-install.sh && ./docker-install.sh
#   Unattended:   ./docker-install.sh --path /opt/layercove --port 8000 --yes
#
# Options:
...
show_help() {
    echo "LayerCove Docker Installation Script"
    echo ""
    ...
    echo "    ./docker-install.sh --path /srv/layercove --port 3000 --tz America/New_York --yes"
    echo ""
...
create_install_dir() {
    ...
    if ! mkdir -p "$INSTALL_PATH" 2>/dev/null; then
        # The default `/opt/layercove` (and any other root-owned parent) needs
...
download_compose_file() {
    ...
    if [[ "$BUILD_FROM_SOURCE" == "true" ]]; then
        ...
        curl -fsSL -o docker-compose.yml \
            https://raw.githubusercontent.com/Timpan4/layercove/main/docker-compose.yml
    fi
...
create_env_file() {
    ...
    cat > .env << EOF
# LayerCove Docker Configuration
# Generated by docker-install.sh on $(date)

# Port LayerCove runs on
PORT=$PORT
...
start_container() {
    ...
    while [[ $attempt -lt $max_attempts ]]; do
        ...
        if $DOCKER_CMD ps | grep -q "Up"; then
            log_success "LayerCove container is running"
            return 0
        fi
