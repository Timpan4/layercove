...
# LayerCove calls `tailscale status` / `tailscale cert` via the host's socket,
...
# LayerCove itself was built from.
COPY .git/HEAD ./.git/HEAD

# Copy built frontend from builder stage
COPY --from=frontend-builder /app/static ./static

# Copy embedded GCode viewer static assets (PrettyGCode + LayerCove adapter).
